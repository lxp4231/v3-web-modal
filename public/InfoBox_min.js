...
(function () {
  ...
  (InfoBox.prototype.getOffset = function () {
    ...
    baidu.object.extend(InfoBox.prototype, {
       ...
       */
      _adjustPosition(poi) {
        const pixel = this._getPointPosition(poi);
        const icon = this._marker && this._marker.getIcon();
      ...
      _panBox() {
        ...
        const mapH = parseInt(this._map.getContainer().offsetHeight, 10);
        const mapW = parseInt(this._map.getContainer().offsetWidth, 10);
        ...
        const anchorPos = this._map.pointToPixel(this._point);
        let panTop;
        let panBottom;
        let panY;
        // 左侧超出
        const panLeft = boxW / 2 - anchorPos.x;
        // 右侧超出
        const panRight = boxW / 2 + anchorPos.x - mapW;
        if (this._marker) {
       ...
       */
      _dispatchEvent(instance, type, opts) {
        type.indexOf('on') != 0 && (type = `on${type}`);
        const event = new baidu.lang.Event(type);
        if (opts) {
          for (const p in opts) {
            event[p] = opts[p];
          }
